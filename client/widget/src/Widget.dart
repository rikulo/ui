 ...
 */
typedef EventListener DomEventDispatcher(Widget target, String type);
...
class Widget implements EventTarget {
	...
	void removeChildHTML_(Widget child) {
		...
		if (n != null) {
			...
			if (!child.prolog.isEmpty() && _isProlog(sib))
				sib.remove();
		} else
	...
	DomEventDispatcher getDomEventDispatcher_(String type) {
		if (_domEvtDisps == null) {
			...
			DomEventDispatcher disp = (Widget target, String type) {
				return (Event event) {
	...
	void domListen_(Element n, String type, DomEventDispatcher disp) {
		final EventListener ln = disp(this, type);
