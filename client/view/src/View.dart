...
*/
	...
	void insertBefore(View child, View beforeChild) {
		...
		if (inDocument)
			insertChildToDocument_(child, beforeChild);
	...
	/** Inserts the HTML content generated by the given child view before the reference view (beforeChild).
	 ...
	 */
	void insertChildToDocument_(View child, View beforeChild) {
		String html = child._asHTML();
		Element before, cave;
		if (beforeChild != null)
		...
		if (before != null) {
			var sib = before.previousNode;
			String txt;
			if (sib is Text && (txt=sib.wholeText) != null && txt.trim().isEmpty())
				before = sib;
